...
export function ServerStats({ servers }: ServerStatsProps) {
  ...
  const sseServers = servers.filter(s => s.type === McpServerType.SSE).length;
    ...
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-6">
      <Card className="bg-white/50 dark:bg-slate-900/70 dark:border-slate-800">
        <CardContent className="p-4 flex justify-between items-center">
          <div>
            <p className="text-sm text-muted-foreground">{t('mcpServers.stats.totalServers')}</p>
      ...
      <Card className="bg-white/50 dark:bg-slate-900/70 dark:border-slate-800">
        <CardContent className="p-4 flex justify-between items-center">
          <div>
            <p className="text-sm text-muted-foreground">{t('mcpServers.stats.active')}</p>
      ...
      <Card className="bg-white/50 dark:bg-slate-900/70 dark:border-slate-800">
        <CardContent className="p-4 flex justify-between items-center">
          <div>
            <p className="text-sm text-muted-foreground">{t('mcpServers.stats.stdio')}</p>
      ...
      <Card className="bg-white/50 dark:bg-slate-900/70 dark:border-slate-800">
        <CardContent className="p-4 flex justify-between items-center">
          <div>
            <p className="text-sm text-muted-foreground">{t('mcpServers.stats.sse')}</p>
            <p className="text-2xl font-bold">{sseServers}</p>
          </div>
